    ...
    <style>
        ...
        * {
        ...
        @keyframes pulse {
            ...
            50% { transform: scale(1.05); box-shadow: 0 8px 25px rgba(79, 70, 229, 0.5); }
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarContent">
                <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="#"><i class="fas fa-calendar-alt nav-icon"></i><span>Schedule</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#"><i class="fas fa-question-circle nav-icon"></i><span>Help</span></a>
                    </li>
                </ul>
                ...
                <div class="ms-auto">
                    ...
                    @php
                        ...
                        $userPhoto = ($user && isset($user->photo) && $user->photo) ? ($baseUrl.'/storage/'.$user->photo) : ('https://ui-avatars.com/api/?name='.urlencode($userName).'&background=4f46e5&color=fff&size=64');
                        $userStatusAnggota = ($user && isset($user->statusanggota)) ? ($user->statusanggota == 'belum' ? 'Anggota' : $user->statusanggota) : 'Guest';
                        $userStatusNextStep = ($user && isset($user->statusnextstep)) ? $user->statusnextstep : 'Guest';
                    @endphp
                            ...
                            <li class="dropdown-header">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted">{{ ($user && isset($user->email)) ? $user->email : 'guest@example.com' }}</small>
                                    </div>
                                </div>
                            </li>
    ...
    <div class="main-container" style="margin-top: 100px;">
        <div class="container">
            ...
            <div class="hero-section">
                ...
                {{-- <p class="hero-subtitle">Platform kelas pemuridan dengan materi terstruktur, interaktif, dan berstandar profesional.</p> --}}
            ...
            @if($user)
                @php
                    ...
                    if($userAccessRequest && isset($userAccessRequest->status)) {
                        if($userAccessRequest->status === 'pending') {
                            ...
                            $showBanner = $hoursSinceApproved <= 24;
                            $bannerType = 'approved';
                            ...
                            $showBanner = $hoursSinceRejected <= 12;
                            $bannerType = 'rejected';
                        }
                    }
                @endphp
                ...
                @if($showBanner)
                    <div class="status-banner {{ $bannerType }}-banner mb-4" id="statusBanner">
                        <div class="banner-content">
                            ...
                            <div class="banner-text">
                                @if($bannerType === 'pending')
                                    ...
                                    <p class="banner-description">Permohonan akses Anda untuk <strong>Discipless Community Class</strong> sedang dalam proses review oleh tim admin. Mohon tunggu notifikasi selanjutnya.</p>
                                @elseif($bannerType === 'approved')
                                    <h5 class="banner-title">Permintaan Akses Diterima!</h5>
                                    <p class="banner-description">Selamat! Permohonan akses Anda untuk <strong>Discipless Community Class</strong> telah disetujui. Anda sekarang dapat mengakses kelas tersebut.</p>
                                @elseif($bannerType === 'rejected')
                                    <h5 class="banner-title">Permintaan Akses Ditolak</h5>
                                    <p class="banner-description">Mohon maaf, permohonan akses Anda untuk <strong>Discipless Community Class</strong> tidak dapat disetujui saat ini. Silakan hubungi admin untuk informasi lebih lanjut.</p>
                                @endif
                            </div>
                            ...
                            @elseif($bannerType === 'rejected')
                                <div class="banner-action">
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#requestAccessModal">
            ...
            @php $user = Auth::user(); @endphp
                ...
                <div class="course-card serve-category"
                    @if($user)
                        @php
                            $userAccessRequest = null;
                            if($user && isset($user->id)) {
                ...
                >
                    ...
                    <div class="course-content">
                        ...
                        <div class="course-status">
                            @php
                                $isCTDM = isset($user) && $user && isset($user->statusanggota) && in_array($user->statusanggota, ['Core Team', 'DM']);
                            @endphp
                            ...
                            <div class="progress-indicator">
                                ...
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width: {{ $isCTDM ? '75%' : '0%' }}"></div>
                                </div>
                            </div>
                        </div>
                        @if($user && isset($isCTDM) && $isCTDM)
                            @if(isset($userAccessRequest) && $userAccessRequest && isset($userAccessRequest->status) && $userAccessRequest->status === 'pending')
                                <div class="request-btn-container">
                                    <a href="#" class="request-btn pending">
                                        <i class="fas fa-clock request-btn-icon"></i> Sedang Diproses
                                    </a>
                                </div>
                            @elseif(isset($userAccessRequest) && $userAccessRequest && isset($userAccessRequest->status) && $userAccessRequest->status === 'rejected')
                                <div class="request-btn-container">
                                    <a href="#" class="request-btn rejected" data-bs-toggle="modal" data-bs-target="#requestAccessModal">
                ...
                <div class="course-card new-category"
                    @php
                        $userAccessRequestNew = null;
                        $canAccessNew = false;
                        if(isset($user) && $user){
                            $userAccessRequestNew = \App\Models\Fc1McRequest::where('user_id', $user->id)
                                ->where('status', '!=', 'rejected')
                                ->latest()
                                ->first();
                            $isApprovedNew = $userAccessRequestNew && isset($userAccessRequestNew->status) && $userAccessRequestNew->status === 'approved';
                ...
                >
                    ...
                    <div class="course-content">
                        ...
                        <div class="course-status">
                            <span class="status-badge {{ $canAccessNew ? 'status-available' : 'status-locked' }}">{{ $canAccessNew ? 'Available' : 'Locked' }}</span>
                            <div class="progress-indicator">
                                <span>Progress</span>
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width: {{ $canAccessNew ? '30%' : '0%' }}"></div>
                                </div>
                            </div>
                        </div>
                        @if(isset($user) && $user && !$canAccessNew)
                            @if($userAccessRequestNew && isset($userAccessRequestNew->status) && $userAccessRequestNew->status === 'pending')
                                <div class="request-btn-container">
                                    <a href="#" class="request-btn pending">
                                        <i class="fas fa-clock request-btn-icon"></i> Sedang Diproses
                                    </a>
                                </div>
                            @elseif($userAccessRequestNew && isset($userAccessRequestNew->status) && $userAccessRequestNew->status === 'rejected')
                                <div class="request-btn-container">
                                    <a href="#" class="request-btn rejected" data-bs-toggle="modal" data-bs-target="#requestFc1McModal" onclick="openFc1McModal()">
                                        <i class="fas fa-redo request-btn-icon"></i> Request Ulang
                                    </a>
                                </div>
                            @else
                                <div class="request-btn-container">
                                    <a href="#" class="request-btn" data-bs-toggle="modal" data-bs-target="#requestFc1McModal" onclick="openFc1McModal()">
                                        <i class="fas fa-plus-circle request-btn-icon"></i> Request Kelas
                                    </a>
                                </div>
                            @endif
                ...
                <div class="course-card plan-category"
...
<div class="modal fade" id="requestFc1McModal" tabindex="-1" aria-labelledby="requestFc1McModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      <form method="POST" action="{{ route('fc1mc.requests.store') }}" id="requestFc1McForm">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <label for="alasanFc1Mc" class="form-label">Pilih Kelas <span class="text-danger">*</span></label>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" id="submitBtnFc1Mc">
            <i class="fas fa-paper-plane me-2"></i>Kirim Permintaan
                ...
                <!-- ESC EQUIP - Grow Class - Kategori: Grow -->
                <div class="course-card grow-category"
                    @if($user && isset($user->statusnextstep) && in_array($user->statusnextstep, ['grow','fruitfull']))
                ...
                >
                    ...
                    <div class="course-content">
                        ...
                        <div class="course-status">
                            @php
                                $canAccessGrow = $user && isset($user->statusnextstep) && in_array($user->statusnextstep, ['grow','fruitfull']);
                            @endphp
                            <span class="status-badge {{ $canAccessGrow ? 'status-available' : 'status-locked' }}">{{ $canAccessGrow ? 'Available' : 'Locked' }}</span>
                            <div class="progress-indicator">
                                <span>Progress</span>
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width: {{ $canAccessGrow ? '30%' : '0%' }}"></div>
                                </div>
                            </div>
                        </div>
                        @if($user && !$canAccessGrow)
                            <div class="request-btn-container">
                                <div class="access-info">
                                    <i class="fas fa-tools text-warning me-2"></i>
                                    <span class="text-muted">Kelas belum bisa diakses - Masih dalam tahap pengembangan</span>
                ...
                <div class="course-card serve-category"
                    @if($user && isset($user->statusnextstep) && $user->statusnextstep === 'fruitfull')
                        onclick="window.location='{{ route('equip') }}'"
                    @endif
                >
                    <div class="course-image">
                    ...
                    <div class="course-content">
                        ...
                        <div class="course-status">
                            ...
                            @endphp
                            <span class="status-badge {{ $canAccessFruitful ? 'status-available' : 'status-locked' }}">{{ $canAccessFruitful ? 'Available' : 'Locked' }}</span>
                            <div class="progress-indicator">
                                <span>Progress</span>
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width: {{ $canAccessFruitful ? '15%' : '0%' }}"></div>
                                </div>
                            </div>
                        </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <form method="POST" action="{{ route('request-access.store') }}" id="requestAccessForm">
            @csrf
            <div class="modal-header">
              <h5 class="modal-title" id="requestAccessModalLabel">Request Access to <span id="courseName">Kelas</span></h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <div class="mb-3">
                <label for="alasan" class="form-label">Kelas yang akan diikuti? <span class="text-danger">*</span></label>
              ...
              <div id="infoBox" class="alert alert-info" style="display: none;">
                <i class="fas fa-info-circle me-2"></i>
                <strong>Info:</strong> Permohonan Anda akan diproses secepatnya oleh tim admin. Mohon tunggu notifikasi selanjutnya.
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
