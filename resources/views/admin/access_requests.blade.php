...
<style>
    ...
    /* User info container untuk positioning badge */
    .user-info-container {
        position: relative;
        ...
        <div class="col-lg-12">
            <div class="glass-card">
                <div class="solid-header p-4 d-flex align-items-center gap-2">
                    ...
                    <span class="solid-title">Permintaan Akses Kelas</span>
                </div>
                        ...
                        <table class="table table-glass align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="user-info-container">
                                                ...
                                                @else
                                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($req->user->name ?? '-') }}&background=7c3aed&color=fff&size=64" class="user-avatar-mini" alt="User">
                                                @endif
                                                @php
                                                    // Tentukan apakah user baru (request dibuat dalam 24 jam terakhir DAN masih pending)
                                                    $isNewUser = $req->created_at->diffInHours(now()) <= 24 && $req->status === 'pending';
                                                @endphp
                                                @if($isNewUser)
                                    ...
                                    <td>
                                        @if($req->status === 'pending')
                                            <form method="POST" action="{{ route('admin.access-requests.approve', $req->id) }}" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn-approve" onclick="return confirm('Apakah Anda yakin ingin menyetujui permintaan ini?')">
                                                    <i class="fas fa-check me-1"></i>Approve
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('admin.access-requests.reject', $req->id) }}" class="d-inline ms-1">
                                                @csrf
